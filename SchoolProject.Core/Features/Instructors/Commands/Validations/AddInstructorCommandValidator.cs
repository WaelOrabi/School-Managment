...
{
    public class AddInstructorCommandValidator : AbstractValidator<AddInstructorCommandModel>
    {
        #region Fields
        ...
        #endregion
        #region Constructor
        public AddInstructorCommandValidator(IStringLocalizer<SharedResources> localizer, IInstructorService instructorService, IDepartmentService departmentService)
        {
        ...
        {
            RuleFor(x => x.NameAr).MustAsync(async (key, CancellationToken) => !await _instructorService.IsNameArExist(key)).WithMessage(_localizer[SharedResourcesKeys.IsExist]);
            RuleFor(x => x.NameEn).MustAsync(async (key, CancellationToken) => !await _instructorService.IsNameEnExist(key)).WithMessage(_localizer[SharedResourcesKeys.IsExist]);
